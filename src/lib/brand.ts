// Logos da Copa. Hospedados externamente por ora; pra produção, baixar e
// commitar em public/brand e trocar por '/brand/...'.
export const LOGO_TROPHY_SRC = 'https://i.postimg.cc/022srSL1/26taca.png'

// Emblema oficial da Copa em BRANCO (pro bloco preto do scoreboard).
export const COPA_LOGO_WHITE_SRC =
  'https://wqnsdhqqbxhdjwnhdeej.supabase.co/storage/v1/object/public/LOGOS/copabranco.png'

// Emblema "We Are 26" em versão monocromática — preta pro modo claro, branca
// pro modo escuro (a logo troca conforme o tema).
export const EMBLEM_SRC_LIGHT = 'https://i.postimg.cc/7Z5MHB9G/weare26-preta.png'
export const EMBLEM_SRC_DARK = 'https://i.postimg.cc/fLfY21W4/weare26-branca.png'

// "Lanterninha": foto do último colocado (ranking geral e pontuação ao vivo).
export const DONKEY_SRC = 'https://i.postimg.cc/mZ74L5sB/burrinho.png'

